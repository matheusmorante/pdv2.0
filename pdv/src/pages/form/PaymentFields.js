...
const PaymentFields = () => {
    const [payments, setPayments] = useState([{ value: "0,00", fee: "0,00", status: '' }])
    ...
    const addPayment = () => {
        setPayments(prev => [...prev, { value: "0,00", fee: "0,00", status: '' }])
    }
        ...
        <table>
            ...
            {payments.map((payment, idx) => (
                <tr>
                    ...
                    <td><input value={payment.value} onChange={e => changePayments(idx, 'value', e.target.value)}/></td>
                    <td><input value={payment.fee } onChange={e => changePayments(idx, 'fee', e.target.fee)}/> %</td>
