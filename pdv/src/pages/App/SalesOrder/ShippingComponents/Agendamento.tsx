...
const Agendamento = ({ scheduling, onChangeScheduling, errors }: AgendamentoProps) => {
    ...
    return (
        <div className="flex-1 flex flex-col min-w-0">
            ...
            <div className={`bg-white dark:bg-slate-900 border p-4 sm:p-5 lg:p-6 rounded-3xl sm:rounded-[2rem] shadow-sm w-full transition-all ${hasError ? 'border-red-500 ring-1 ring-red-500/20' : 'border-slate-100 dark:border-slate-800'}`}>
                <div className="flex flex-col sm:flex-row flex-wrap items-start sm:items-center gap-4 relative group w-full">
                    ...
                    <div className="flex-1 min-w-[140px] w-full relative">
                        <input
                            type="date"
                            className={`w-full bg-transparent border-0 border-b p-2 outline-none text-sm transition-all dark:text-slate-300 ${errors['shipping_date'] ? 'border-red-500' : 'border-slate-200 dark:border-slate-800 focus:border-blue-600 dark:focus:border-blue-500'}`}
                    ...
                    {scheduling.type === 'range' ? (
                        <div className="flex flex-1 flex-row items-center gap-3 relative group/time min-w-[200px] w-full">
                            <input
                                type="time"
                                className={`w-full bg-transparent border-0 border-b p-2 outline-none text-sm transition-all dark:text-slate-300 ${errors['shipping_time'] ? 'border-red-500' : 'border-slate-200 dark:border-slate-800 focus:border-blue-600 dark:focus:border-blue-500'}`}
