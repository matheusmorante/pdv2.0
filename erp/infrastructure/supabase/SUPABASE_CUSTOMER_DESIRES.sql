...
DROP POLICY IF EXISTS "Permitir inserção para autenticados" ON public.customer_desires;
CREATE POLICY "Permitir inserção para autenticados" ON public.customer_desires FOR INSERT TO authenticated WITH CHECK (true);

DROP POLICY IF EXISTS "Permitir leitura para autenticados" ON public.customer_desires;
CREATE POLICY "Permitir leitura para autenticados" ON public.customer_desires FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "Permitir atualização para autenticados" ON public.customer_desires;
