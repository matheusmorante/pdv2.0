...
DROP POLICY IF EXISTS "Permitir inserção para autenticados" ON public.attendance_logs;
CREATE POLICY "Permitir inserção para autenticados" ON public.attendance_logs FOR INSERT TO authenticated WITH CHECK (true);

DROP POLICY IF EXISTS "Permitir leitura para autenticados" ON public.attendance_logs;
