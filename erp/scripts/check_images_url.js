...
async function check() {
    const { data, error } = await supabase
        .from('products')
        .select('description, main_image_url')
    ...
    if (error) {
        ...
        console.log(`Found ${data.length} products with a main image url.`);
        data.forEach(p => console.log(`- ${p.description}: ${p.main_image_url}`));
    }
    process.exit(0);
}
