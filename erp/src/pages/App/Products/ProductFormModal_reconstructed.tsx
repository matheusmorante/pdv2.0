        ...
        <td className="px-6 py-4">
            <div className="flex items-center gap-2">
                ...
                <input
                    ...
                    className={`w-20 bg-transparent border-none outline-none text-sm font-black ${v.syncCostPrice ? 'text-slate-400' : 'text-slate-700 dark:text-slate-200'}`}
                />
            </div>
        </td>
        <td className="px-6 py-4">
            ...
            <div className="flex items-center justify-center gap-2">
                {isCombo && (
                    <button
                        ...
                        title="Composi├º├úo do Combo para esta varia├º├úo"
                    >
...
const ProductFormModal = ({ isOpen, onClose, product, initialData, onSuccess }: ProductFormModalProps) => {
    ...
    useEffect(() => {
        ...
        // Se for um produto novo ou j├í for um rascunho
    ...
    useEffect(() => {
        ...
        const parentChanged =
            formData.unitPrice !== prevParentValues.current.unitPrice ||
            formData.costPrice !== prevParentValues.current.costPrice ||
            formData.freightCost !== prevParentValues.current.freightCost ||
            formData.freightType !== prevParentValues.current.freightType ||
            formData.ipiPercent !== prevParentValues.current.ipiPercent;
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement> | React.DragEvent) => {
        ...
        if (files.length + externalUrls.length + (formData.images?.length || 0) > 15) {
            toast.error("Limite m├íximo de 15 fotos atingido.");
            return;
        }
        ...
        try {
            ...
            for (const url of externalUrls) {
                try {
                    ...
                    toast.warning("N├úo foi poss├¡vel capturar esta imagem da web. Salve a foto no seu computador e arraste o arquivo.");
                }
            }
    ...
    const generateBulkVariations = () => {
        ...
        toast.success(`${newVariations.length} varia├º├Áes geradas com sucesso!`);
    };
        ...
        setFormData(prev => {
            const newVariations = prev.variations?.map(v => {
                if (v.id === id) {
                    ...
                    // If manually editing price, disable sync
                    if (field === 'unitPrice') updated.syncUnitPrice = false;
                    if (field === 'costPrice') updated.syncCostPrice = false;
                    return updated;
                }
                return v;
            });
    ...
    const handleGenerateCategory = async () => {
        if (!formData.description && !formData.ecommerceDescription) {
            toast.warning("Preencha o t├¡tulo ou a descri├º├úo do produto para sugerir categorias.");
            return;
        }
        ...
        try {
            ...
            const prompt = `Baseado no t├¡tulo "${formData.description}" e na descri├º├úo "${formData.ecommerceDescription || formData.whatsappDescription || ''}", identifique as 1 a 3 categorias mais adequadas dentre as seguintes op├º├Áes (RETORNE APENAS OS IDS SEPARADOS POR V├ìRGULA):\n${categoriesPrompt}`;

            const response = await aiService.chat(prompt, "Sempre retorne apenas IDs separados por v├¡rgula. Sem explica├º├Áes ou sauda├º├Áes.");
            ...
            if (suggestedIds.length > 0) {
                ...
                toast.info("A IA n├úo conseguiu identificar uma categoria correspondente clara.");
            }
        } catch (error) {
    ...
    const handleGenerateComboName = async () => {
        ...
        const loadingToast = toast.loading("­ƒñû Gerando um nome profissional para o seu combo...");

        try {
            const itemsList = formData.comboItems.map(i => `${i.quantity}x ${i.description}`).join(", ");
            const prompt = `Voc├¬ ├® um especialista em marketing de m├│veis. Crie um NOME COMERCIAL (t├¡tulo) curto, profissional e atraente para um jogo/combo que cont├®m os seguintes itens: ${itemsList}.
            Retorne APENAS o nome gerado, sem aspas, sem explica├º├Áes e sem sauda├º├Áes. Exemplo: Jogo de Cozinha Completa 5 Pe├ºas - Linha Premium.`;

            const data = await aiService.chat(prompt, "Sempre retorne apenas o nome do produto. Sem explica├º├Áes.");
            setFormData(prev => ({ ...prev, description: data.answer.trim().replace(/^"|"$/g, '') }));
            toast.update(loadingToast, { render: "Nome do combo gerado com sucesso! Ô£¿", type: "success", isLoading: false, autoClose: 3000 });
        } catch (error: any) {
    ...
    const handleGenerateAIDescription = async (channel: 'ecommerce' | 'whatsapp') => {
        if (!formData.description) {
            toast.warning("Preencha o 'T├¡tulo do Produto' na aba Cadastro Geral primeiro.");
            setActiveTab('geral');
            return;
        }
        ...
        const loadingToast = toast.loading(`­ƒñû Gerando c├│pia para ${channelLabel}... Aguarde.`);
        ...
        try {
            ...
            let finalPrompt = "";
            if (isWhatsapp) {
                finalPrompt = `Crie uma descri├º├úo seguindo este template base, preenchendo as informa├º├Áes do produto.
                INSTRU├ç├òES IMPORTANTES:
                1. USE APENAS TEXTO SIMPLES E EMOJIS.
                2. N├âO USE NENHUMA TAG HTML (como <b>, <ul>, etc).
                3. SUBSTITUA as tags entre colchetes ou chaves (ex: [NOME], {price}) pelos valores REAIS.

                TEMPLATE PARA SEGUIR:
                ${basePrompt || "[T├ìTULO DO PRODUTO]\nPre├ºo: [PRE├çO]\n..."}

                DADOS DO PRODUTO:
                T├¡tulo: ${formData.description}
                Linha/Modelo: ${formData.line || "N├úo informado"}
                Diferencial Principal: ${formData.mainDifferential || "N├úo informado"}
                Material: ${formData.material || "N├úo informado"}
                Cores Dispon├¡veis: ${formData.colors || "N├úo informado"}
                O que N├âO acompanha: ${formData.notIncluded || "Nada consta"}
                ...
                Pre├ºo: R$ ${formData.unitPrice || 0}`;
            } else {
                finalPrompt = `Crie uma descri├º├úo de alto impacto para E-commerce seguindo RIGOROSAMENTE esta estrutura HTML:

                1. T├ìTULO OTIMIZADO (h1): [Produto] + [Marca] + [Modelo] + [Diferencial Principal] + [Cor].
                2. TEXTO DE APRESENTA├ç├âO: Um par├ígrafo persuasivo (copy) focado em resolver problemas e benef├¡cios emocionais.
                3. DESTAQUES E DIFERENCIAIS: Use <ul> e <li> para listar os principais pontos fortes.
                4. FICHA T├ëCNICA: Uma <table> formatada com bordas simples contendo detalhes t├®cnicos (Medidas, Material, etc).

                REGRAS:
                - Use apenas tags HTML b├ísicas (<h1>, <p>, <ul>, <li>, <table>, <tr>, <td>, <b>).
                - Baseie os detalhes t├®cnicos no TEMPLATE BASE abaixo.
                - N├úo inclua markdown (como \`\`\`html).

                TEMPLATE BASE / FONTE DE DADOS:
                ${basePrompt}

                DADOS DO PRODUTO:
                T├¡tulo do Produto: ${formData.description}
                Linha/Modelo: ${formData.line || "N├úo informado"}
                Diferencial Principal: ${formData.mainDifferential || "N├úo informado"}
                Material: ${formData.material || "N├úo informado"}
                Cores Dispon├¡veis: ${formData.colors || "N├úo informado"}
                O que N├âO acompanha: ${formData.notIncluded || "Nada consta"}
                Dimens├Áes: ${formData.width || '?'}cm (L) x ${formData.height || '?'}cm (A) x ${formData.depth || '?'}cm (P)
                ${formData.extraDimensions?.map(ed => `${ed.label}: ${ed.value}`).join('\n                ') || ''}
                Categoria: ${formData.category || "Geral"}
                Pre├ºo: R$ ${formData.unitPrice || 0}`;
            }
            ...
            toast.update(loadingToast, { render: `Descri├º├úo para ${channelLabel} gerada! Ô£¿`, type: "success", isLoading: false, autoClose: 3000 });
        } catch (error: any) {
    ...
    const handleGenerateNCM = async () => {
        if (!formData.description) {
            toast.warning("Preencha o 'T├¡tulo do Produto' na aba Cadastro Geral primeiro para eu entender do que se trata o produto.");
            setActiveTab('geral');
            return;
        }

        const loadingToast = toast.loading("­ƒñû A IA est├í buscando o c├│digo NCM... Aguarde.");
        try {
            ...
            toast.update(loadingToast, { render: "NCM localizado com IA! Ô£¿", type: "success", isLoading: false, autoClose: 3000 });
        } catch (error: any) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.description) {
            toast.error("O t├¡tulo do produto ├® obrigat├│rio.");
            return;
        }
                    ...
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            <h2 className="text-2xl font-black text-slate-800 dark:text-slate-100 tracking-tight flex items-center gap-3">
                                {product ? 'Editar' : 'Novo'} {formData.isCombo ? 'Combo/Jogo' : formData.itemType === 'service' ? 'Servi├ºo' : 'Produto'}
                            </h2>
                            <p className="text-[10px] uppercase font-black text-slate-400 dark:text-slate-500 tracking-widest mt-1">Preencha as informa├º├Áes abaixo</p>
                        </div>
                        {isSavingDraft && (
                    ...
                    <div className="flex gap-8">
                        {[
                            ...
                            ...(formData.itemType === 'product' ? [
                                ...
                                { id: 'variacoes', label: 'Varia├º├Áes', icon: 'bi-grid-3x3-gap' },
                            ] : []),
                            { id: 'ecommerce', label: 'Marketplace/Ecommerce (Informa├º├Áes Espec├¡ficas)', icon: 'bi-cart-check' },
                            { id: 'fiscal', label: 'Tribut├írio / NF', icon: 'bi-file-earmark-text' },
                        ].map(tab => (
                            <button
                                key={tab.id}
                    ...
                    {activeTab === 'geral' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                            <div className="md:col-span-2 relative group uppercase font-black">
                                <div className="flex items-center justify-between mb-1">
                                    <label className="text-[10px] uppercase font-black text-slate-400 dark:text-slate-500 tracking-widest">
                                        T├¡tulo do Produto <span className="text-red-500">*</span>
                                    </label>
                                    {formData.isCombo && (
                                        <button
                                            type="button"
                                ...
                                <input
                                    ...
                                    placeholder={formData.isCombo ? "Ex: Jogo de Jantar Moderno 4 Cadeiras" : "Ex: Camiseta Algod├úo Eg├¡pcio Premium"}
                                />
                            </div>

                            <div className="grid grid-cols-2 gap-6 md:col-span-2">
                                <SmartInput
                                    label="C├│digo (SKU Principal)"
                                    value={formData.code}
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={() => {
                                                            setFormData(prev => {
                                                                ...
                                                                const nextIds = ids.includes(parent.id) ? ids.filter(i => i !== parent.id) : [...ids, parent.id];
                                                                
                                                                // Update category string field for backward compatibility
                                                                let mainCategoryName = prev.category || "";
                                                                if (nextIds.length > 0) {
                                                                    const firstCat = availableCategories.find(c => c.id === nextIds[0]);
                                                                    if (firstCat) mainCategoryName = firstCat.name;
                                                                } else {
                                                                    mainCategoryName = "";
                                                                }

                                                                return { ...prev, categoryIds: nextIds, category: mainCategoryName };
                                                            });
                                                        }}
                                ...
                                {formData.itemType === 'product' && (
                                    <div className="flex flex-col gap-2 md:col-span-2 mt-2">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">
                                            Condi├º├úo do Produto <span className="text-red-500">*</span>
                                        </label>
                            ...
                            <div className="flex flex-col gap-2 md:col-span-2">
                                <div className="flex items-center justify-between">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">Pre├ºo de Venda (R$)</label>
                                    {formData.isCombo && (
                                        <button
                                            type="button"
                                            onClick={() => {
                                                ...
                                                toast.info(`Pre├ºo calculado: R$ ${total.toFixed(2)}`);
                                            }}
                            ...
                            <div className="md:col-span-2 mt-4 bg-slate-50/50 dark:bg-slate-950/20 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 flex flex-col gap-8">
                                <div className="flex items-center justify-between">
                                    <h4 className="text-xs font-black uppercase tracking-widest text-slate-800 dark:text-slate-200 flex items-center gap-2">
                                        <i className="bi bi-rulers text-blue-600"></i> Detalhes T├®cnicos / Dimens├Áes
                                    </h4>
                                    <p className="text-[9px] text-slate-400 uppercase font-black tracking-widest">Essas informa├º├Áes enriquecem a descri├º├úo gerada pela IA</p>
                                </div>

                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    {/* Material selection */}
                                    <div className="flex flex-col gap-4">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Material do M├│vel</label>
                                        <div className="grid grid-cols-2 gap-2">
                                            {['MDP', 'MDF', 'MDP/MDF', 'Mad. Maci├ºa', 'Metal/MDP', 'Metal', 'Outro'].map(mat => (
                                                <button
                                                    key={mat}
                                                    type="button"
                                                    onClick={() => {
                                                    ...
                                                    className={`px-3 py-2 rounded-xl text-[10px] font-bold border transition-all ${((mat !== 'Outro' && formData.material === mat) || (mat === 'Outro' && !['MDP', 'MDF', 'MDP/MDF', 'Mad. Maci├ºa', 'Metal/MDP', 'Metal'].includes(formData.material || ''))) ? 'bg-blue-600 text-white border-blue-600' : 'bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 text-slate-500 hover:border-blue-300'}`}
                                                >
                                                    {mat}
                                                </button>
                                            ))}
                                        </div>
                                        {(!['MDP', 'MDF', 'MDP/MDF', 'Mad. Maci├ºa', 'Metal/MDP', 'Metal'].includes(formData.material || '') || formData.material === '') && (
                                            <input
                                    ...
                                    <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-slate-100 dark:border-slate-800">
                                        ...
                                        <div className="flex flex-col gap-2">
                                            ...
                                            <input
                                                ...
                                                placeholder="Ex: Dobradi├ºas com amortecimento"
                                                className="w-full px-4 py-3 bg-white dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-xl text-xs font-bold"
                                            />
                                        </div>

                                        <div className="flex flex-col gap-2">
                                            <label className="text-[9px] font-black uppercase tracking-widest text-slate-400">Cores Dispon├¡veis</label>
                                            <input
                                        ...
                                        <div className="flex flex-col gap-2">
                                            <label className="text-[9px] font-black uppercase tracking-widest text-slate-400">O que N├âO acompanha</label>
                                            <input
                                    ...
                                    {/* Dimensions */}
                                    <div className="flex flex-col gap-4">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Dimens├Áes Totais (cm)</label>
                                ...
                                <div className="flex flex-col gap-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                                    <div className="flex items-center justify-between">
                                        <div className="flex flex-col">
                                            <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Medi├º├Áes Adicionais (M├íx. 10)</label>
                                            <p className="text-[9px] text-slate-400 italic">Ex: "Altura at├® o assento", "80cm"</p>
                                        </div>
                                        <button
                                            type="button"
                                            onClick={() => {
                                                if ((formData.extraDimensions?.length || 0) >= 10) {
                                                    toast.warning("Limite de 10 medi├º├Áes extras atingido.");
                                                    return;
                                                }
                                        ...
                                        >
                                            <i className="bi bi-plus-lg mr-1"></i> Adicionar Medi├º├úo
                                        </button>
                                    </div>
                                    ...
                                    <div className="flex flex-col gap-3">
                                        {formData.extraDimensions?.map((ed, idx) => (
                                            <div key={idx} className="grid grid-cols-1 md:grid-cols-[1fr,1fr,auto] gap-3 items-end animate-in fade-in slide-in-from-top-1 duration-200">
                                                <div className="flex flex-col gap-1.5">
                                                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-tighter">O que est├í medindo?</span>
                                                    <input
                                                        value={ed.label}
                                                        onChange={(e) => {
                                                        ...
                                                        placeholder="Ex: Altura at├® o assento"
                                        ...
                                        {(formData.extraDimensions?.length || 0) === 0 && (
                                            <p className="text-[10px] text-slate-400 italic bg-slate-50 dark:bg-slate-900/30 p-4 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800 text-center">
                                                Nenhuma medi├º├úo extra adicionada. Clique em "Adicionar Medi├º├úo" para enriquecer a descri├º├úo.
                                            </p>
                                        )}
                                    </div>
                                    <p className="text-[9px] text-slate-400 italic">Use para detalhes extras como "Profundidade esticado", "Altura do bra├ºo", etc.</p>
                                </div>
                            </div>
                                ...
                                <div className="md:col-span-2 bg-purple-50/30 dark:bg-purple-900/5 p-6 rounded-[2.5rem] border border-purple-100 dark:border-purple-900/20 flex flex-col gap-4">
                                    <div className="flex items-center justify-between">
                                        <h4 className="text-xs font-black uppercase tracking-widest text-purple-800 dark:text-purple-200 flex items-center gap-2">
                                            <i className="bi bi-layers-fill"></i> Composi├º├úo do Combo
                                        </h4>
                                        <p className="text-[9px] text-purple-600 uppercase font-black tracking-widest italic">O estoque do combo ser├í calculado automaticamente.</p>
                                    </div>
                        ...
                        <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                <div className="bg-slate-50/50 dark:bg-slate-900/50 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 flex flex-col gap-6">
                                    <h4 className="text-xs font-black uppercase tracking-widest text-slate-800 dark:text-slate-200 flex items-center gap-2">
                                        <i className="bi bi-tag text-blue-600"></i> Composi├º├úo de Custo
                                    </h4>

                                    <div className="flex flex-col gap-2">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">Pre├ºo Base de Custo</label>
                                        <input
                                            type="number"
                                            step="0.01"
                                    ...
                                    <div className="mt-2 p-4 bg-emerald-50 dark:bg-emerald-900/10 rounded-2xl border border-emerald-100 dark:border-emerald-900/30">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-emerald-600 dark:text-emerald-400 block mb-1">Pre├ºo de Compra Final</label>
                                ...
                                <div className="bg-slate-50/50 dark:bg-slate-900/50 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 flex flex-col gap-6">
                                    <h4 className="text-xs font-black uppercase tracking-widest text-slate-800 dark:text-slate-200 flex items-center gap-2">
                                        <i className="bi bi-stack text-blue-600"></i> Gest├úo de Invent├írio
                                    </h4>
                                    ...
                                    <div className="flex flex-col gap-2">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 flex items-center gap-2">
                                            Quantidade Inicial
                                            {!!product && <span className="text-[8px] text-amber-500 font-bold lowercase tracking-normal">(use o bot├úo de "Estoque" na lista para alterar)</span>}
                                        </label>
                                        <input
                                            type="number"
                                    ...
                                    <div className="flex flex-col gap-2">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">Fornecedor Padr├úo</label>
                                        <select
                                    ...
                                    <div className="flex flex-col gap-2">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">Estoque de Alerta (M├¡nimo)</label>
                                        <input
                                            type="number"
                                    ...
                                    <div className="mt-2 p-4 bg-blue-50 dark:bg-blue-900/10 rounded-2xl border border-blue-100 dark:border-blue-900/30">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-blue-600 dark:text-blue-400 block mb-1">
                                            {formData.isCombo ? "Estoque Calculado (M├¡nimo dos Itens)" : "Status do Estoque Atual"}
                                        </label>
                                ...
                                <div className="bg-amber-50/50 dark:bg-amber-900/10 p-6 rounded-3xl border border-amber-100 dark:border-amber-900/30 flex flex-col gap-5">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h4 className="text-xs font-black uppercase tracking-widest text-slate-800 dark:text-slate-200">Configura├º├úo de Notifica├º├Áes</h4>
                                            <p className="text-[10px] text-slate-500 dark:text-slate-400 font-medium mt-0.5">
                                                ...
                                                {formData.condition === 'salvado' && (
                                                    <span className="ml-1 font-black text-amber-600 dark:text-amber-400">
                                                        (Produto salvado ÔÇö sem alertas por padr├úo)
                                                    </span>
                                                )}
                                            </p>
                                        </div>
                                    ...
                                    >
                                        <div>
                                            <p className="text-xs font-black text-slate-800 dark:text-slate-100">Notifica├º├Áes Ativadas</p>
                                        ...
                                        <div className="flex flex-col gap-3">
                                            ...
                                            >
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <p className="text-xs font-black text-slate-800 dark:text-slate-100">Alerta de Estoque M├¡nimo</p>
                                                        <p className="text-[10px] text-slate-500 font-medium mt-0.5">
                                                            Notificar quando atingir o limite m├¡nimo
                                                            {(formData.minStock || 0) > 0 ? ` (atual: ${formData.minStock} ${formData.unit || 'un'})` : ' (defina o estoque m├¡nimo acima)'}
                                                        </p>
                                                    </div>
                                                </div>
                        ...
                        <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                            <div className="flex items-center justify-between bg-blue-50 dark:bg-blue-900/10 p-6 rounded-3xl border border-blue-100 dark:border-blue-900/30">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h4 className="text-sm font-black text-blue-900 dark:text-blue-200">Habilitar Grade de Varia├º├Áes</h4>
                                        <p className="text-[10px] text-blue-600 uppercase font-black tracking-widest mt-0.5">Permite criar varia├º├Áes de cor, tamanho, etc.</p>
                                    </div>
                                </div>
                                {formData.hasVariations && (
                                    <div className="flex items-center gap-3">
                                        ...
                                        >
                                            <i className="bi bi-plus-lg mr-2"></i> Adicionar Varia├º├úo
                                        </button>
                                    </div>
                                )}
                            </div>
                            ...
                            {isBulkGeneratorOpen && (
                                <div className="bg-slate-50 dark:bg-slate-900/40 p-8 rounded-[2rem] border-2 border-dashed border-blue-200 dark:border-blue-900/30 animate-in zoom-in-95 duration-300">
                                    <div className="flex items-center justify-between mb-6">
                                        <div>
                                            <h4 className="text-sm font-black text-slate-800 dark:text-slate-100 uppercase tracking-tight">Gerador Autom├ítico de Grade</h4>
                                            <p className="text-[10px] text-slate-500 dark:text-slate-400 font-medium mt-1">Defina os atributos e separe os valores por v├¡rgula para gerar todas as combina├º├Áes.</p>
                                        </div>
                            ...
                            {formData.hasVariations ? (
                                <div className="border border-slate-100 dark:border-slate-800 rounded-[2rem] overflow-hidden bg-white dark:bg-slate-950 shadow-sm">
                                    <table className="w-full text-left">
                                        <thead className="bg-slate-50 dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800">
                                            <tr>
                                                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400">Nome da Varia├º├úo</th>
                                                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400">SKU / C├│digo</th>
                                                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400 text-center">Sinc. Tudo</th>
                                                <th className="px-6 py-4 text-left text-[10px] font-black uppercase tracking-widest text-slate-400">R$ Custo</th>
                                                <th className="px-6 py-4 text-left text-[10px] font-black uppercase tracking-widest text-slate-400">R$ Venda</th>
                                                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400">Estoque</th>
                                                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400">Est. M├¡n.</th>
                                                ...
                                                <tr>
                                                    <td colSpan={8} className="px-6 py-12 text-center">
                                                        <div className="flex flex-col items-center gap-2 text-slate-400">
                                                            ...
                                                            <p className="text-xs font-bold">Nenhuma varia├º├úo cadastrada.</p>
                                                        </div>
                                                    </td>
                                                </tr>
                                            )}
                                ...
                                <div className="border-2 border-dashed border-slate-100 dark:border-slate-800 rounded-[2.5rem] p-12 text-center bg-slate-50/20 dark:bg-slate-900/10">
                                    ...
                                    <p className="text-slate-300 dark:text-slate-700 text-[10px] mt-1 uppercase tracking-widest font-black">Habilite as varia├º├Áes no bot├úo acima se este produto tiver cores, tamanhos ou modelos diferentes.</p>
                                </div>
                            )}

                            {/* Modal de Composi├º├úo da Varia├º├úo (Combo) */}
                                        ...
                                        <div className="p-8 border-b border-slate-50 dark:border-slate-800 flex items-center justify-between">
                                            <div>
                                                <h3 className="text-xl font-black text-slate-800 dark:text-slate-100 tracking-tight flex items-center gap-3 uppercase">
                                                    ...
                                                    Composi├º├úo da Varia├º├úo: <span className="text-purple-600">{formData.variations?.find(v => v.id === editingVariationComboId)?.name}</span>
                                                </h3>
                                                <p className="text-[10px] uppercase font-black text-slate-400 dark:text-slate-500 tracking-widest mt-1 italic">Defina quais itens comp├Áem esta vers├úo espec├¡fica do combo.</p>
                                            </div>
                                            <button
                                            ...
                                            <div className="mt-8 flex justify-end">
                                                <button
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        toast.info(`Pre├ºo da varia├º├úo atualizado: R$ ${total.toFixed(2)}`);
                                                    }}
                                                ...
                                                >
                                                    <i className="bi bi-calculator"></i> Somar Itens e Atualizar Pre├ºo
                                                </button>
                                            </div>
                                        </div>
                        ...
                        <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                            ...
                            <div className="flex gap-4 p-1 bg-slate-100 dark:bg-slate-950/50 rounded-2xl self-start">
                                {[
                                    ...
                                    { id: 'descriptions', label: 'Descri├º├úo / Canais', icon: 'bi-pencil-square' },
                                ].map(tab => (
                                    <button
                                        key={tab.id}
                                        type="button"
                                ...
                                >
                                    <div className="flex items-center justify-between">
                                        <div>
                                            ...
                                            <p className="text-[10px] text-slate-400 uppercase font-black tracking-widest mt-1">At├® 15 fotos ÔÇö arraste e solte aqui ou clique em "Enviar Foto"</p>
                                        </div>
                                        <div className={`text-[10px] font-black uppercase px-3 py-1 rounded-full transition-colors ${isDraggingPhoto ? 'text-green-700 bg-green-100 dark:bg-green-900/30' : 'text-blue-600 bg-blue-50 dark:bg-blue-900/20'}`}>
                                            {isDraggingPhoto ? '­ƒôé Solte aqui!' : `${formData.images?.length || 0} / 15`}
                                        </div>
                                    </div>
                            ...
                            {activeEcommerceSubTab === 'descriptions' && (
                                <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 animate-in fade-in slide-in-from-left-4 duration-300">
                                    ...
                                    <div className="flex flex-col gap-6 bg-green-50/40 dark:bg-green-900/5 p-6 rounded-[2.5rem] border border-green-100 dark:border-green-900/30">
                                        ...
                                        <div className="flex flex-col gap-3">
                                            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-1.5">
                                                <i className="bi bi-pencil-square text-green-500" /> Recheio da Descri├º├úo:
                                            </p>
                                            <textarea
                                                rows={10}
                                                value={formData.whatsappDescription || ''}
                                                onChange={(e) => setFormData({ ...formData, whatsappDescription: e.target.value })}
                                                placeholder="Aqui aparecer├í o conte├║do gerado pela IA..."
                                    ...
                                    <div className="flex flex-col gap-6 bg-blue-50/30 dark:bg-blue-900/5 p-6 rounded-[2rem] border border-blue-100 dark:border-blue-900/30">
                                        ...
                                        <div className="flex flex-col gap-3">
                                            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-1.5">
                                                <i className="bi bi-pencil-square text-blue-600" /> Descri├º├úo:
                                            </p>
                                            <textarea
                                                rows={17}
                                                value={formData.ecommerceDescription || ''}
                                                onChange={(e) => setFormData({ ...formData, ecommerceDescription: e.target.value })}
                                                placeholder="Descreva as especifica├º├Áes para o Ecommerce. Use HTML (<b>, <ul>, etc) para formatar."
                    ...
                    {activeTab === 'fiscal' && (
                        <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                            <div className="bg-amber-50 dark:bg-amber-900/10 p-6 rounded-3xl border border-amber-100 dark:border-amber-900/30 flex items-start gap-4">
                                ...
                                <div>
                                    <h4 className="text-xs font-black uppercase tracking-widest text-amber-800 dark:text-amber-200">Informa├º├Áes Fiscais para NF-e</h4>
                                    <p className="text-[10px] text-amber-600 uppercase font-black tracking-widest mt-0.5">Estes dados s├úo essenciais para a emiss├úo autom├ítica de notas fiscais.</p>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                ...
                                <div className="relative">
                                    <div className="flex items-center justify-between mb-2">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">NCM (8 D├¡gitos)</label>
                                        <button
                                ...
                                <div className="flex flex-col gap-2">
                                    ...
                                    >
                                        ...
                                        <option value="1">1 - Estrangeira (Importa├º├úo Direta)</option>
                                        <option value="2">2 - Estrangeira (Mercado Interno)</option>
                                        <option value="3">3 - Nacional (Superior a 40% conte├║do importa├º├úo)</option>
                                    </select>
                                </div>

                                <div>
                                    <SmartInput
                                        label="Situa├º├úo Tribut├íria (CST/CSOSN)"
                ...
                <div className="p-8 border-t border-slate-50 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/30 flex justify-between gap-3 shrink-0">
                    ...
                    <div className="flex gap-3">
                        ...
                        >
                            ...
                            {product ? "Salvar Altera├º├Áes" : "Concluir Cadastro"}
                        </button>
                    </div>
                </div>
            </div>
